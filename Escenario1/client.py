...
# Función para generar llaves públicas y privadas
def diffie_hellman_keypair(p, g):
    private_key = get_random_bytes(16)  # Generamos una llave privada aleatoria
    public_key = pow(g, int.from_bytes(private_key, 'big'), p)  # Calculamos la llave pública
    return private_key, public_key

# Función para generar la llave simétrica compartida
def generate_shared_key(private_key, public_key_received, p):
    shared_secret = pow(public_key_received, int.from_bytes(private_key, 'big'), p)
...
# Generar las llaves del cliente (privada y pública)
private_key_client, public_key_client = diffie_hellman_keypair(p, g)
